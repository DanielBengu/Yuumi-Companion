...
namespace TransparentOverlay_simpleExample
{
    ...
    {
        BackgroundWorker bwMaster = new BackgroundWorker();
        BackgroundWorker bwAPI = new BackgroundWorker();
        ...
        private void AddCardToList()
        {
            ...
            if (this.EnemyCardList.InvokeRequired)
            {
                AddCardCallback d = new AddCardCallback(AddCardToList);
                this.Invoke(d, new object[] { });
            }
            else
            {
                List<Card> cardList = RetrieveData();
                this.EnemyCardList.DataSource = cardList.Select(card => card.name).ToList();
                this.pictureBox1.ImageLocation = cardList.Skip(rand.Next(0, 400)).First().assets.First().gameAbsolutePath;
            }
        }
    }
}